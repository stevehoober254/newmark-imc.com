...
<div class="card">
    ...
    <div class="card-body">
        <form method="POST" action="{{ route("admin.clients.update", [$client->id]) }}" enctype="multipart/form-data">
            ...
            <div class="form-group">
                <label for="client_url">{{ trans('cruds.client.fields.client_url') }}</label>
                <input class="form-control {{ $errors->has('client_url') ? 'is-invalid' : '' }}" type="text" name="client_url" id="client_url" value="{{ old('client_url', $client->client_url) }}">
                @if($errors->has('client_url'))
                    <div class="invalid-feedback">
                        {{ $errors->first('client_url') }}
                    </div>
                @endif
                <span class="help-block">{{ trans('cruds.client.fields.client_url_helper') }}</span>
            </div>
            <div class="form-group">
